...
covid_conf_ts = pd.read_csv("time_series_covid19_confirmed_global.csv")
covid_dead_ts = pd.read_csv('time_series_covid19_deaths_global.csv')
covid_recv_ts = pd.read_csv('time_series_covid19_recovered_global.csv')
...
def generate_cards(cntry='US'):
    ...
    cards = html.Div(
        [
            dbc.Row(
                [
                    dbc.Col(dbc.Card(generate_card_content("Recovered",recv_cntry_total,recv_overall_total), color="green", inverse=True),md=dict(size=2,offset=3)),
                    dbc.Col(dbc.Card(generate_card_content("Confirmed",conf_cntry_total,conf_overall_total), color="yellow", inverse=True),md=dict(size=2)),
                    dbc.Col(dbc.Card(generate_card_content("Dead",dead_cntry_total,dead_overall_total),color="red", inverse=True),md=dict(size=2)),
                ],
                className="mb-4",
            ),
        ],id='card1'
